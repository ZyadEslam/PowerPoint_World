...
const SlideInNotifications = () => {
  ...
  return (
    <div className="bg-white min-h-[200px] flex items-center justify-center">
      <button
        onClick={() => {
          setNotifications((pv: { id: number, text: string }[]) => [generateRandomNotif(), ...pv]);
        }}
      ...
      <div className="flex flex-col gap-1 w-72 fixed top-2 right-2 z-50 pointer-events-none">
        <AnimatePresence>
          {notifications.map((n: { id: number, text: string }) => (
            <Notification removeNotif={removeNotif} text={n.text} id={n.id} key={n.id} />
          ))}
        </AnimatePresence>
      </div>
    </div>
...
const Notification = ({ text, id, removeNotif }: { text: string, id: number, removeNotif: (id: number) => void }) => {
  useEffect(() => {
  ...
  return (
    ...
    >
      <CheckSquareIcon className=" mt-0.5" />
      <span>{text}</span>
